...
import Svg.Styled as Svg exposing (Svg, g, polygon, svg)
import Svg.Styled.Attributes exposing (fill, height, points, transform, viewBox)
...
modelHeart : Model
modelHeart =
    { tb1 = Static -160 120 0
    , tb2 = Static 150 -90 180
    , tm = Static -270 -93 45
    , sqr = Static 0 -300 0
    , par = Static 231 91 0
    , ts1 = Static 150 224 0
    , ts2 = Static -106 -150 90
    }
...
view : List (Svg.Attribute msg) -> Model -> Html msg
view attrs model =
    svg
    ...
    case shape of
        ...
        Moving vx vy va x y a tx ty ta ->
            let
                ...
                ax =
                    pstiffness * (tx - x) - pdamping * vx

                ay =
                    pstiffness * (ty - y) - pdamping * vy

                at =
                    astiffness * (tt - a) - adamping * va
